...
# Modern laboratuvar ve soft mavi-beyaz arka plan
background_image = "https://images.unsplash.com/photo-1509228468518-180dd4864904?auto=format&fit=crop&w=1500&q=80"
st.markdown(
    f"""
    <style>
    .stApp {{
        background: linear-gradient(135deg, #f0f4f8 0%, #ddeffd 100%), url("{background_image}");
    ...
    /* Bildirim kutularını kontrastlı ve okunaklı yap */
    .stAlert-success {{
...
# --- Versiyon Bilgisi ---
st.sidebar.markdown("---")
st.sidebar.info("🛠 Versiyon: 1.0.0\n📅 Güncelleme: 2025-05-25\n📌 Modern renkler, okunaklı bildirimler, GC modülü ve Solvent paneli")
...
if MODUL == _("GC Formülasyon Karşılaştırma", "GC Formulation Comparison"):
    ...
    if st.button(_("Formülasyonu Hesapla", "Calculate Formulation")):
        ...
        st.info(_("- Şu anki karışım VP: **{:.2f} mmHg**".format(mevcut_vp), "- Current mix VP: **{:.2f} mmHg**".format(mevcut_vp)))
        st.info(_("- Hedeflenen karışım VP: **{:.2f} mmHg**".format(hedef_vp), "- Target mix VP: **{:.2f} mmHg**".format(hedef_vp)))
